...
void s21::Token::Validator(std::string input, std::string input_x) {
  ...
  input_x = 1;
  input = Token.ConvertToLower(input);
  ...
  for (size_t index = 0; input.length() > index; ++index){
    std::string token = Token.ReadToken(input, index);
    if(!isdigit(token.at(0))) Token.TryToPushTokenToStack(token);
    else Token.PushNumberToStack(token, stod(token));
  }
...
double s21::Token::PostfixNotationCalculation(double x_value) {
  using namespace s21;
